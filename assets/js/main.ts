...
class ServerWorksWebsite {
  ...
  constructor() {
    this.contactForm = document.getElementById('contactForm') as ContactForm;
      ...
      this.navLinks.forEach(link => {
        link.addEventListener('click', (e: Event) => {
          ...
          if (target && target.startsWith('#')) {
            ...
            if (element) {
              this.smoothScrollTo(element as HTMLElement);
            }
          }
        });
      });
  ...
  private setupNavbarScrollEffect(): void {
    let lastScrollTop = 0;
  ...
  private validateField(field: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement): void {
  ...
  private updateActiveNavLink(activeLink: HTMLAnchorElement): void {
    this.navLinks.forEach(link => {
